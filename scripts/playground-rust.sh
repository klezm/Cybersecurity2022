...
if [ $WRITE_LOG == "true" ]; then
    ...
    cargo +nightly -C $REPO_NAME $VERBOSE_2 $IS_LOCKED fuzz run $VERBOSE_1 $FUZZING_TARGET $CORPUS_DIR -- -only_ascii=$ONLY_ASCII -seed=$SEED -max_total_time=$MAX_TOTAL_TIME -ignore_crashes=1
    ...
    # `-- -error_exitcode=77`: Exit code (default 77) used if libFuzzer itself (not a sanitizer) reports a bug (leak, OOM, etc).
    # `-- -print_final_stats=0`: If 1, print statistics at exit. Defaults to 0.
    # `-- -fork=N`: Experimental mode -fork=N (where N is the number of parallel jobs) enables oom-, timeout-, and crash-resistant fuzzing with separate processes (using fork-exec, not just fork).
    # `-- -ignore_crashes=0`: False by default, same as -ignore_ooms, but for all other crashes.
    # `-- -`:
    # `-- -`:
    # `-- -`:
fi
